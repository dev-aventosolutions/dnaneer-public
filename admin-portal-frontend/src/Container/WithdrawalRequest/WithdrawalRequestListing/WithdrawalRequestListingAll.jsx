...
import WithdrawalRequestModalInstitutional from "../WithdrawalRequestModalInstitutional";

const WithdrawalRequestListingAll = ({ request }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const { data } = await getWithdrawalRequestListing("all");
        if (data) {
          ...
          const finalData = data?.data[0]?.map((investor) => {
            return {
              ...
              mode: investor?.mode,
              status: investor?.status.toUpperCase(),
  ...
  return (
    <Spin spinning={loading}>
      {data?.user_type === 1 ? (
        ...
        <WithdrawalRequestModalInstitutional
          visible={visible}
          handleOk={handleOk}
          handleCancel={handleCancel}
          rejection={handleRejectReasonCancel}
...
export default WithdrawalRequestListingAll;
